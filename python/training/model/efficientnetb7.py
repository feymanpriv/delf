...
class efficientNetb7(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.avg_pool = layers.AveragePooling2D((7, 7),
                                            ...
                                            data_format=data_format)
    if self.include_top:
        self.efficientbase = EfficientNetB7(include_top=True, weights='imagenet', input_tensor=None,
                                            input_shape=None, pooling=None)
    else:
        self.efficientbase = EfficientNetB7(include_top=False, weights='imagenet',input_shape=(448,448,3))
  ...
  def build_call(self, inputs, training=True, intermediates_dict=None):
    ...
    x=self.efficientbase(inputs)

    if self.block3_strides:
        x1 = self.subsampling_layer(x)
        if intermediates_dict is not None:
            intermediates_dict['block3'] = x1
    else:
        if intermediates_dict is not None:
            intermediates_dict['block3'] = x
